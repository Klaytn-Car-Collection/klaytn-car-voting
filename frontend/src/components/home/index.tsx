  ...
  return (
    <>
      ...
      {!isLoadingFetchList && data && data.length > 0 && (
        <>
          <div className="z-10 mt-5 font-bold">TOTAL DEPOSITED: {dataTotalVote} CCT</div>
      ...
      <div className="z-10 my-5 grid w-full max-w-screen-xl grid-cols-1 gap-5 px-5 md:grid-cols-2 xl:px-0">
        {data &&
          data.length > 0 &&
          data.map(({ name, images, amountVote, avatar }, index) => (
            <a
              onClick={(e) => {
                e.preventDefault()
                setImageId(index + 1)
              }}
              className="relative flex flex-col rounded-lg bg-gray-500 bg-cover bg-local bg-center bg-no-repeat text-left bg-blend-multiply shadow-md hover:bg-blend-soft-light dark:hover:bg-blend-darken"
              style={{ backgroundImage: `url(${avatar})`, height: '300px' }}
            >
              <Carousel className="absolute h-full rounded-lg" height={'100%'}>
                {images.map((e) => (
                  <img src={e} alt="image" className="h-full w-full object-cover" />
                ))}
              </Carousel>
              <div className="absolute right-0 z-10 flex-1 p-8 text-right text-white">
                <span className="rounded-md bg-white bg-opacity-75 p-2 text-xs text-black">
              ...
              <div className="absolute bottom-0 z-10 p-8 text-white">
                <p className="mb-5 max-w-xl font-extrabold leading-tight tracking-tight drop-shadow-lg">
                  {name}
                </p>
                <div className="mt-5 flex items-center">
                  <button
                    ...
                    className="mr-2 inline-flex shadow-md items-center rounded-lg border border-white bg-white px-2.5 py-1.5 text-center text-xs font-medium text-black hover:text-blue-500 focus:outline-none focus:outline-0 focus:ring-4 focus:ring-gray-700"
                  >
                  ...
                  <div className="text-xs drop-shadow-lg">Min. vote: {MIN_APPROVE_AMOUNT}</div>
                </div>
              </div>
            </a>
          ))}
